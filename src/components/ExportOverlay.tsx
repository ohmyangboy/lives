  ...
  return (
    <div className="overlay" role="dialog" aria-modal="true">
      <div className="export-card">
        {state !== 'running' && <button className="overlay-close" onClick={onClose}><CloseIcon /></button>}
        {state === 'running' && <>
          <div className="processing-orbit"><span /><i /></div>
          <span className="eyebrow">正在生成 Live Photo</span>
          <h2>{labels[stage]}</h2>
          <p>所有处理均在这台 Mac 上完成，原始视频不会被修改。</p>
          <div className="export-progress"><i style={{ width: `${Math.max(3, progress * 100)}%` }} /></div>
        ...
        {state === 'success' && <>
          ...
          <h2>{destination === 'photos' ? '你的 Live Photo 已就绪' : '配对文件已保存'}</h2>
        ...
        {state === 'error' && !isPhotoPermissionError && <>
          ...
          <h2>{message ?? '生成 Live Photo 失败'}</h2>
