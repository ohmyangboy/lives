...
final class MediaEngineTests: XCTestCase {
    func testMidClipTimingWithProvidedRecording() async throws {
        guard let path = ProcessInfo.processInfo.environment["LIVES_EXPORT_TIMING_FIXTURE"] else {
            throw XCTSkip("设置 LIVES_EXPORT_TIMING_FIXTURE 复核录屏中段导出")
        }
        let sourceURL = URL(fileURLWithPath: path)
        let info = try await LivesMediaEngine.inspect(url: sourceURL)
        let asset = SourceAsset(displayName: "recording", relativePath: "recording.mp4", durationMs: info.durationMs,
        ...
        let directory = FileManager.default.temporaryDirectory.appendingPathComponent("LivesTiming-\(UUID())")
        defer { try? FileManager.default.removeItem(at: directory) }
        let pair = try await LivesMediaEngine.render(request: RenderRequest(project: project,
            canvasSize: CanvasSize(width: 360, height: 640), audioPolicy: .muted, dynamicRange: .sdr),
            resolvedURLs: [asset.id: sourceURL], outputDirectory: directory)
        ...
        XCTAssertEqual(range.duration.seconds, 3, accuracy: 1.0 / 30.0 + 0.001)
        ...
        while let sample = output.copyNextSampleBuffer() {
            ...
            lastTime = CMSampleBufferGetPresentationTimeStamp(sample)
            count += 1
        }
        XCTAssertEqual(reader.status, .completed)
        XCTAssertEqual(count, 90, "录屏的时间戳间隙不能丢失输出帧")
        XCTAssertEqual(lastTime.seconds, 89.0 / 30.0, accuracy: 0.001)
    }

    func testDefaultWatermarkIncludesIconButCustomAndDisabledDoNot() throws {
        func pixels(_ mode: WatermarkMode) throws -> [UInt8] {
            let context = try XCTUnwrap(CGContext(data: nil, width: 360, height: 100,
                bitsPerComponent: 8, bytesPerRow: 360 * 4, space: CGColorSpaceCreateDeviceRGB(),
                bitmapInfo: CGImageAlphaInfo.premultipliedLast.rawValue))
            LivesMediaEngine.drawWatermark(settings: WatermarkSettings(mode: mode, text: "lives", opacity: 1),
                                           canvas: CGSize(width: 360, height: 100), context: context)
            let data = try XCTUnwrap(context.data)
            return Array(UnsafeBufferPointer(start: data.assumingMemoryBound(to: UInt8.self), count: 360 * 100 * 4))
        }
        let branded = try pixels(.lives)
        let custom = try pixels(.custom)
        let disabled = try pixels(.none)
        func coloredPixels(_ data: [UInt8]) -> Int {
            stride(from: 0, to: data.count, by: 4).filter {
                Int(max(data[$0], data[$0 + 1], data[$0 + 2])) - Int(min(data[$0], data[$0 + 1], data[$0 + 2])) > 20
            }.count
        }
        XCTAssertGreaterThan(coloredPixels(branded), 5, "默认水印应绘制彩色 App 图标")
        XCTAssertEqual(coloredPixels(custom), 0, "自定义水印仅绘制文字")
        XCTAssertTrue(disabled.allSatisfy { $0 == 0 })
        XCTAssertTrue(custom.contains { $0 != 0 })
    }
    ...
    func testAudioSwitchPreservesAudibleSamplesAndMutesExport() async throws {
        ...
        defer { try? FileManager.default.removeItem(at: directory) }
        for (duration, start) in [(1000, 2000), (2000, 2000), (3000, 2000), (3900, 2000), (3900, 7000)] {
            for enabled in [true, false] {
                let project = ProjectDocument(assets: [asset], placements: [
                    Placement(sourceAssetID: asset.id, slotID: "full", startTimeMs: start, audioEnabled: enabled)
                ...
                let pair = try await LivesMediaEngine.render(
                    ...
                    resolvedURLs: [asset.id: url], outputDirectory: directory.appendingPathComponent("\(duration)-\(start)-\(enabled)"))
                ...
                let track = try XCTUnwrap(tracks.first)
                let range = try await track.load(.timeRange)
                XCTAssertEqual(range.duration.seconds, Double(min(duration, info.durationMs - start)) / 1000, accuracy: 0.08)
                let reader = try AVAssetReader(asset: movie)
                let output = AVAssetReaderTrackOutput(track: track, outputSettings: [
